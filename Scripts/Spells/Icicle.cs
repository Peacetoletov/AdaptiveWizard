...
public class Icicle : StraightProjectile
{
    ...
    private void EvaluateCollisionWithCapsuleCollider() {
        ...
        RaycastHit2D hit = Physics2D.CapsuleCast((Vector2) transform.position + offsetRotated, capsuleCollider.size, CapsuleDirection2D.Horizontal, angle, Vector2.zero, 0, LayerMask.GetMask("Enemy", "Wall"));
    ...
    private void EvaluateCollisionWithBoxCollider() {
        ...
        RaycastHit2D hit = Physics2D.BoxCast((Vector2) boxCollider.transform.position + offsetRotated, boxCollider.size, angle, Vector2.zero, 0, LayerMask.GetMask("Enemy", "Wall"));
        if (hit.collider == null) {
            transform.position += (Vector3) GetDirection().normalized * GetSpeed() * Time.deltaTime;
        }
        else {
