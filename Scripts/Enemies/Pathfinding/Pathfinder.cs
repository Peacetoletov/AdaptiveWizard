...
namespace AdaptiveWizard.Assets.Scripts.Enemies.Pathfinding
{
    public static class Pathfinder
    {
        ...
        public static Path PathUntilFirstTurn(Node start, Node target, EnemyMovement.StuckInfo stuckInfo) {
            ...
            A_Star(start, target.GetPosition(), stuckInfo);                                        // modify nodes
            Path path = CreatePath(target, start.GetPosition());             // create a path from modified nodes
            ResetModifiedNodes();
            return path;
        }
        
        private static void A_Star(Node start, Vector2Int targetPosition, EnemyMovement.StuckInfo stuckInfo) {
        ...
        private static void ProcessFirstNode(PriorityQueue<Node> openNodes, Vector2Int targetPosition, EnemyMovement.StuckInfo stuckInfo) {
            ...
            const int distanceToNeighbour = 10;
        ...
        private static Path CreatePath(Node target, Vector2Int startPos) {
            ...
            int distance = 10 * nodesInOneDirection;
